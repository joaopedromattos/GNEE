...
def cora_networkx(path=None):
    ...
    if path == None:
        ...
        node_data = pd.read_csv(
            os.path.join(path, "cora.content"),
            sep="\t",
            header=None,
            names=column_names,
        )
        ...
        adj = sp.coo_matrix(
            (
                np.ones(converted_edges.shape[0]),
                (converted_edges[:, 0], converted_edges[:, 1]),
            ),
            shape=(len(node_data), len(node_data)),
            dtype=np.float32,
        )
        ...
        node_data["new_index"] = node_data["new_index"].apply(lambda x: idx_map[x])

        # Encoding our labels with categorical values from 0 to 6
        classes_dict = encode_onehot_dict(node_data["subject"].to_numpy())
        node_data["subject"] = node_data["subject"].apply(lambda x: classes_dict[x])
        ...
        idx_train, idx_val, idx_test = np.split(
            idxs, [int(0.6 * num_nodes), int(0.8 * num_nodes)]
        )
    ...
    *args,
    ...
    if not custom_function:
        ...
        if function == cora_networkx or function == None:
            adj, features, labels, idx_train, idx_val, idx_test = cora_networkx(path)
        else:
            adj, features, labels, idx_train, idx_val, idx_test = function(*args)
